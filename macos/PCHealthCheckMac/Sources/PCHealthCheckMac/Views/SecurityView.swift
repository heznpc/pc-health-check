    ...
    var body: some View {
        Form {
            ...
            Section("개인정보 및 외부 연결") {
                HStack(spacing: 12) {
                    NativeStatusGlyph(symbol: "checkmark.shield", tint: .green)
                    VStack(alignment: .leading, spacing: 2) {
                        Text("로컬 진단").font(.body.weight(.medium))
                        Text(
                            model.virusTotalEnabled
                ...
                HStack(spacing: 12) {
                    NativeStatusGlyph(
                        ...
                        tint: model.virusTotalEnabled ? .orange : .gray
                    )
                    VStack(alignment: .leading, spacing: 2) {
                        Text("외부 해시 조회").font(.body.weight(.medium))
                        Text(model.virusTotalEnabled ? "VirusTotal SHA-256 조회가 켜져 있습니다." : "현재 꺼져 있습니다.")
                            .font(.subheadline)
                            .foregroundStyle(.secondary)
                    }
                    Spacer()
                    Text(model.virusTotalEnabled ? "켜짐" : "꺼짐")
                        .foregroundStyle(.secondary)
                }
                .padding(.vertical, 4)
            }
            ...
            if !model.autorunRows.isEmpty {
                Section {
                    ForEach(Array(model.autorunRows.prefix(8))) { row in
                    ...
                    if model.autorunRows.count > 8 {
                        Text("그 외 \(model.autorunRows.count - 8)개")
                            .font(.subheadline)
                            .foregroundStyle(.secondary)
                    }
                } header: {
            ...
            if !model.recentInstalls.isEmpty {
                Section {
                    ForEach(Array(model.recentInstalls.prefix(6))) { install in
                        HStack(alignment: .top, spacing: 12) {
                            NativeStatusGlyph(symbol: "shippingbox", tint: .secondary)
                            VStack(alignment: .leading, spacing: 2) {
                                Text(install.name).font(.body.weight(.medium))
                                Text([install.installDate, install.publisher].filter { !$0.isEmpty }.joined(separator: " · "))
                                    .font(.subheadline)
                                    .foregroundStyle(.secondary)
                            }
                            Spacer()
            ...
            if let issues = model.storage?.accessIssues, !issues.isEmpty {
                Section {
                    ...
                    Button("전체 디스크 접근 권한 열기", systemImage: "gear") {
            ...
            Section {
                ...
                NativeSectionHeader(
                    title: "진단 결과",
                    subtitle: model.summary?.message ?? "",
                    value: "\(model.summary?.warningCount ?? 0)건 확인"
                )
            }
        }
